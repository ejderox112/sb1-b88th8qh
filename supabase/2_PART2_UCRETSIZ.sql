-- =====================================================
-- ÜCRETSİZ SUPABASE İÇİN DÜZELTİLMİŞ PART2
-- =====================================================
-- Not: pg_notify gibi premium özellikler kaldırıldı
...
CREATE INDEX idx_business_ads_status ON business_ads(status);
...
CREATE POLICY "indoor_photos_moderate" ON indoor_photos FOR UPDATE USING (
  ...
  EXISTS (
    ...
    AND user_profiles.user_role IN ('admin', 'moderator')
  )
);
...
-- =====================================================
-- FONKSİYONLAR (ÜCRETSİZ SÜRÜM İÇİN DÜZELTİLDİ)
-- =====================================================

-- Reklam izleme XP (pg_notify kaldırıldı)
...
-- Reklam izlenme kaydı (earth_distance yerine basit hesaplama)
...
BEGIN
  ...
  IF p_user_latitude IS NOT NULL AND p_user_longitude IS NOT NULL THEN
    v_distance_meters := 111320 * SQRT(
      POW(v_ad_location.latitude - p_user_latitude, 2) + 
      POW((v_ad_location.longitude - p_user_longitude) * COS(RADIANS(v_ad_location.latitude)), 2)
    );
  END IF;
...
-- İçerik raporlama (pg_notify kaldırıldı)
...
GRANT SELECT ON business_ads TO authenticated;
GRANT SELECT ON ad_interactions TO authenticated;
...
SELECT 'ÜCRETSİZ SUPABASE İÇİN PART 2 KURULUMU TAMAMLANDI! ✅' AS status;

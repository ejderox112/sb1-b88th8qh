...
// ... (SVG icon and LoadingIndicator components remain the same) ...
const ChatIcon = () => (
...
const ChatBot: React.FC<ChatBotProps> = ({ lineColor }) => {
  ...
  // Use the correct environment variable for Expo
  ...
  return (
    <>
      <button onClick={() => setIsOpen(!isOpen)} className="fixed bottom-6 right-6 z-50 p-4 rounded-full bg-gray-800 border border-gray-600 text-white hover:bg-gray-700 transition-all shadow-lg" style={{ borderColor: lineColor }} aria-label="Toggle Chatbot">
        <ChatIcon />
      </button>

      {isOpen && (
        <div className="fixed bottom-24 right-6 w-96 h-[32rem] bg-gray-900/80 backdrop-blur-md rounded-lg shadow-2xl flex flex-col z-50 border border-gray-700 overflow-hidden" style={{ borderColor: lineColor, animation: 'fade-in 0.3s ease-out' }}>
          ...
          <form onSubmit={handleSendMessage} className="p-4 border-t border-gray-700 shrink-0" style={{ borderColor: lineColor }}>
            <div className="flex space-x-2">
              <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Ask something..." className="w-full p-2 bg-gray-800 rounded border border-gray-600 focus:ring-2 focus:outline-none focus:border-current" style={{color: lineColor, '--tw-ring-color': lineColor} as React.CSSProperties} disabled={isLoading} />
              <button type="submit" className="p-2 bg-blue-800/70 hover:bg-blue-700 rounded text-white font-bold disabled:opacity-50" disabled={isLoading}>Send</button>
            </div>
          </form>
        </div>
      )}
